Electric Boiler
---------------

.. list-table:: Asset Attributes
   :widths: 35 25 60 25 25 35 70
   :header-rows: 1

   * - ESDL attribute category
     - ESDL attribute
     - Definition

       (as interpreted by the simulator / optimizer)
     - Units
     - Required/Optional
     - Default value

       (in the front end)
     - Optimizer use
   * - Basic attributes
     - Name
     - Asset unique name
     - \-
     - Required
     - Generic unique name generated by the front end
     - \-
   * - Basic attributes
     - State
     - States if the asset is ENABLED or OPTIONAL
     - \-
     - Required
     - ENABLED
     - ENABLED:
       use the specific asset with all its specified attributes

       OPTIONAL:
       allows the optimizer to determine if the asset is required

       (if not then it will be removed from the network), else if

       the asset is required then it will be sized appropriately

   * - Basic attributes
     - Technical Lifetime
     - Operational lifetime of the asset (min=1, max 50)
     - Years
     - Optional
     - \-
     - Once-off costs occur once per lifetime span
   * - Basic attributes
     - Power
     - Size of the heat pump in terms electical power input.

       Must be > 0.0
     - Watts
     - Required
     - \-
     - If state=OPTIONAL: the Power is used as the upper limit for sizing the asset

   * - Basic attributes
     - Efficiency
     - The efficiency of the conversion of commodity to the heat.

       Heat loss in the asset = Efficiency - 1.0.

       Value must be <= 1.
     - \-
     - Optional
     - \-
     - Default value of Efficiency is 1.0 in optimizer if nothing is specified in the ESDL

   * - Ports
     - Inport and OutPort (1 of each)
     - 1 inport and 1 outport for  the heat (production) side.

       The heat producing should be connected to 1 hydraulically coupled network.

       Each port must have a carrier assigned to it.

       Port naming convention required: In & Out

       The heat producing side:  :math:`T_{Out} > T_{In}`
     - \-
     - Required
     - \-
     - \-
   * - Cost Information
     - Investment Costs
     - CAPEX: Once-off cost linked to the size (heat produced) of the asset
     - EUR/MW or
       EUR/kW
     - Required
     - \-
     - \-
   * - Cost Information
     - Installation Costs
     - CAPEX: Once-off cost
     - EUR
     - Required
     - \-
     - \-
   * - Cost Information
     - Variable Operational Costs
     - OPEX: Cost linked to the consumed electric energy over time
     - EUR/MWh
     - Required
     - \-
     - \-
   * - Cost Information
     - Fixed Maintenance Costs
     - OPEX: Cost per year linked to the size (heat produced)  of the asset
     - EUR/MW or
       EUR/MW/yr or
       EUR/kW or
       EUR/kW/yr
     - Optional
     - \-
     - \-
   * - Cost Information
     - Fixed Operational Costs
     - OPEX: Cost per year linked to the size  (heat produced)  of the asset
     - EUR/MW or
       EUR/MW/yr or
       EUR/kW or
       EUR/kW/yr
     - Optional
     - \-
     - \-


Electric Boiler from EDR Assets
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The Design Toolkit also provides access to predefined electric boiler assets from
`ESDL Energy Data Repository (EDR) <https://edr.hesi.energy/>`_

* `ElectricBoiler: Electric boiler <https://edr.hesi.energy/cat/Assets/%2Fedr%2FPublic%2FAssets%2FDistrict%20heating%20and%20cooling%20networks%2FElectric%20boiler.edd>`_

