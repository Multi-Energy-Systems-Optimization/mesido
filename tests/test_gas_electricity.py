...
class TestGasElect(TestCase):
    def test_gas_elect(self):
        """
        Small scaled case that uses gas and electricity commodities is added. Case includes 2 Heating demand,
        1 gas pipe looped network, 1 electricity network, 2 heat pumps and 2 gas boilers. Conversion assets
        have installation (EUR), investment (EUR/MW) and variable operational costs (EUR/MWh). Gas pipes and
        electricity cables have investment cost (EUR/m). Units in EUR/MW and EUR/MWh of conversion assets
        refers to per-watt-electricity or per-watt-gas or per=watt-thermal depending on the commodity that is
        consumed or produced by the conversion asset.

        Checks:
        1. the power recieved by heat demand is equal to the power supplied by conversion assets
        2. the result dH is equal to manually calculated dH via linear interpolation.
        3. gas consumption is equal to production
        4. manually calculated TCO is equal to Objective function value
        """
