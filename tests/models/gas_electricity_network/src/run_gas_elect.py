        ...
        if asset in solution.energy_system_components["heat_source"]:
            ...
            if parameters[f"{asset}.diameter"] > 0:
                for iter in range(len(pipe_classes)):
                    ...
                    if abs(pipe_classes[iter].inner_diameter - parameters[f"{asset}.diameter"]) < 0.01:  # pipe_classes[iter].inner_diameter == parameters[f"{asset}.diameter"]:
                        investment_cost = (
