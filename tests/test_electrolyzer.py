...
class TestElectrolyzer(TestCase):
    def test_electrolyzer_inequality(self):
        ...
        pipe_id = name_to_id_map["Pipe_6ba6"]
        gas_demand_id = name_to_id_map["GasDemand_0cf3"]
        elec_demand_id = name_to_id_map["ElectricityDemand_9d15"]
        electrolyzer_id = name_to_id_map["Electrolyzer_fc66"]
        wind_park_id = name_to_id_map["WindPark_7f14"]
        gas_storage_id = name_to_id_map["GasStorage_e492"]
        ...
        v_inspect = results[f"{pipe_id}.GasOut.Q"] / solution.parameters(0)[f"{pipe_id}.area"]
        head_loss_max = darcy_weisbach.head_loss(
            solution.gas_network_settings["maximum_velocity"],
            solution.parameters(0)[f"{pipe_id}.diameter"],
            ...
            pressure=solution.parameters(0)[f"{pipe_id}.pressure"],
        )
        for iv in range(len(v_inspect)):
            ...
            np.testing.assert_allclose(
                -results[f"{pipe_id}.dH"][iv], 2.1760566566624733, rtol=1e-6, atol=1e-12
            )
        ...
        ub = solution.get_timeseries(f"{wind_park_id}.maximum_electricity_source").values
        np.testing.assert_array_less(results[f"{wind_park_id}.ElectricityOut.Power"], ub + tol)

        # Check that the wind farm setpoint matches with the production
        np.testing.assert_allclose(
            results[f"{wind_park_id}.ElectricityOut.Power"],
            ub * results[f"{wind_park_id}__set_point"],
        )

        # Checks on the storage
        timestep = 3600.0
        np.testing.assert_allclose(
            np.diff(results[f"{gas_storage_id}.Stored_gas_mass"]),
            results[f"{gas_storage_id}.Gas_tank_flow"][1:] * timestep,
            rtol=1e-6,
            atol=1e-8,
        )
        ...
        a, b = solution._get_linear_coef_electrolyzer_mass_vs_epower_fit(
            ...
            electrical_power_min=max(
                solution.parameters(0)[f"{electrolyzer_id}.minimum_load"],
                0.01 * solution.bounds()[f"{electrolyzer_id}.ElectricityIn.Power"][1],
            ),
        ...
        np.testing.assert_allclose(
            results[f"{electrolyzer_id}.ElectricityIn.Power"],
        ...
        gas_tranport_cost = sum(
            ...
            / 3600.0
            * results[f"{pipe_id}.GasOut.mass_flow"][1:]
            * 0.1,
        )
        np.testing.assert_allclose(
            gas_tranport_cost,
        ...
        np.testing.assert_allclose(
            storage_fixed_opex,
            sum(results[f"{gas_storage_id}__fixed_operational_cost"]),
        )
    ...
    def test_electrolyzer_minimum_power(self):
        ...
        np.testing.assert_allclose(
            results[f"{electrolyzer_id}.ElectricityIn.Power"][-1],
            0.0,
            atol=5e-5,
        )
        # Check that the output gas is 0
        np.testing.assert_allclose(
            results[f"{electrolyzer_id}.Gas_mass_flow_out"][-1],
            0.0,
        )
    ...
    def test_electrolyzer_constant_efficiency(self):
        ...
        # Electrolyser
        efficiency = solution.parameters(0)[f"{electrolyzer_id}.efficiency"]
        np.testing.assert_allclose(
            results[f"{electrolyzer_id}.Gas_mass_flow_out"] * efficiency * 3600,
            results[f"{electrolyzer_id}.ElectricityIn.Power"],
        )
    ...
    def test_electrolyzer_equality_constraint(self):
        ...
        for idx in range(3):
            np.testing.assert_allclose(
                results[f"{electrolyzer_id}.Gas_mass_flow_out"][idx],
                results[f"{electrolyzer_id}.ElectricityIn.Power"][idx] * a[idx] + b[idx],
            )
        # Check hardcoded values
        np.testing.assert_allclose(
            results[f"{electrolyzer_id}.Gas_mass_flow_out"],
