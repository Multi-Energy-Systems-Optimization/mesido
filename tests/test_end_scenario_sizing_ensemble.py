...
class TestEndScenarioSizingEnsemble(TestCase):
    def test_end_scenario_sizing_staged(self):
        ...
        inv_costs_ates, opex_ates = check_consistent_size_and_costs(ates, equal_size_assets=True)
        ...
        for h_p in heat_problem.energy_system_components.get("heat_pipe", []):
            ...
            for e_m in range(heat_problem.ensemble_size):
                tech_life_time = parameters[e_m][f"{h_p}.technical_life"]
                pipe_inv = results[e_m][heat_problem._asset_investment_cost_map[h_p]][0]
                pipe_inst = results[e_m][heat_problem._asset_installation_cost_map[h_p]][0]
                asset_state = heat_problem.parameters(e_m)[f"{h_p}.state"]
                if asset_state == AssetStateEnum.OPTIONAL:
                    inv_costs_pipes[e_m] += (pipe_inv + pipe_inst) * max(1.0, problem_years / tech_life_time)
