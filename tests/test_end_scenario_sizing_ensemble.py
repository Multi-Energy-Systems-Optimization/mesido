    ...
    def test_end_scenario_sizing_staged(self):
        ...
        for config in run_asset_map:
                ...
                for prod in assets:
                    ...
                    for e_m in range(heat_problem.ensemble_size):
                        ...
                        if asset_state == AssetStateEnum.OPTIONAL:
                            inv_costs_source[e_m] += inv_cost * max(1.0, problem_years / tech_life_time)
                            inv_costs_source[e_m] += inst_cost * max(1.0, problem_years / tech_life_time)
            ...
            inv_costs_dem, opex_dem = check_consistent_size_and_costs(
                heat_demands, equal_size_assets=True, asset_opex_optional=True
            )
            equal_sizes_heat_source = True if "heat_source" in config["_asset_types_fixed_size"] \
                else False
            inv_costs_source, opex_source = check_consistent_size_and_costs(
                heat_sources, equal_size_assets=equal_sizes_heat_source
            )
            equal_sizes_geo = (
                True if "geothermal" in config["_asset_types_fixed_size"] else False
            )
            _, _ = check_consistent_size_and_costs(
                geothermal, equal_size_assets=equal_sizes_geo
            )
            ...
            equal_sizes_ates = (
                True if "ates" in config["_asset_types_fixed_size"] else False
            )
            inv_costs_ates, opex_ates = check_consistent_size_and_costs(ates, equal_size_assets=equal_sizes_ates)

            inv_costs_pipes, opex_pipes = check_consistent_size_and_costs(
                heat_pipes, equal_size_assets=False, asset_opex_optional=False
            )
