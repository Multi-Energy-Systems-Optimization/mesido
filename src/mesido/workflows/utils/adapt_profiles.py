...
def adapt_profile_to_copy_for_number_of_years(problem, number_of_years: int):
    ...
    for ensemble_member in range(problem.ensemble_size):
        ...
        # Log warnings in case of the extension of leap day/leap year
        original_year = org_timeseries[0].year
        if calendar.isleap(original_year):
            leap_days = [
                datetime.datetime(original_year, 2, 29, 00, 00, 00) + relativedelta(hours=j)
                for j in range(24)
            ]
            # Log warning that the entire year profile will be adapted as leap year profiles as leap
            # year profiles
            if len(org_timeseries) == 8784:
                logger.warning(
                    "The original profile used is of a leap year. The adapted "
                    "profiles for the consecutive years will account for the leap "
                    "day as well."
                )
            # If not a full year profile, log a warning if any leap hour (an hour that falls within
            # the leap day interval) is detected
            elif any(leap_day in org_timeseries for leap_day in leap_days):
                logger.warning(
                    "At least one instance (hour) from the original profile is found. "
                    "The adapted profile for the consecutive years will account for this "
                    "instance as well."
                )
        # If the original year was not a leap year, but if the adaptation will need to include a
        # leap year, log a warning that the leap day in the extended leap year will NOT be
        # considered
        else:
            if len(org_timeseries) == 8760:
                if any(map(calendar.isleap, range(original_year, original_year + number_of_years))):
                    logger.warning(
                        "The adaptation will technically have to include a leap year, but note that"
                        "resulting profile will represent neither datetime nor values for the leap"
                        "day."
                    )
            else:
                pass
        ...
        # the 'n' next (consecutive) leap years. This is because neither of the datetime, dateutils
        # nor the pandas
        def years_required_leap_year(start, n):
            return [y for y in range(start, start + n * 8) if calendar.isleap(y)][:n]

        next_leap_years = years_required_leap_year(2021, 3)
        for year in range(1, number_of_years):
            if year == number_of_years - 1 or skip_last_day is False:
                if calendar.isleap(original_year):
                    new_date_times.extend(
                        [
                            i + relativedelta(years=next_leap_years[year] - original_year)
                            for i in org_timeseries
                        ]
                    )
                else:
                    new_date_times.extend([i + relativedelta(years=year) for i in org_timeseries])
            else:
                if calendar.isleap(original_year):
                    new_date_times.extend(
                        [
