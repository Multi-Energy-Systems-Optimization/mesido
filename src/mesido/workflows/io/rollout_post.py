...
class RollOutPost:
    ...
    def plot_asset_allocation(self):
        ...
        for ates in ates_assets:
            ...
            plt.plot(times / 3600 / 24, np.array(results[f"{ates.name}.Stored_heat"]) / 1e9,
                     label=str(ates.name))
        ...
        for heatsource in heat_sources:
            # stored_heat = [results.get(f"{ates}.Stored_heat", 0) for t in range(times)]
            plt.plot(
                times / 3600 / 24, np.array(results[f"{heatsource.name}.Heat_source"]) / 1e6, label=str(
                    heatsource)
            )
    ...
    def plot_financials(self):
        ...
        for asset in self.esh.get_all_instances_of_type(esdl.EnergyAsset):
            if asset in [*self.esh.get_all_instances_of_type(esdl.HeatingDemand),
                ...
                var_opex_costs = np.array(heat_flow_var[1:]) * variable_operational_cost_coeff * dt/3600
...
if __name__ == "__main__":
    kwargs = {'output_folder':r"C:\git\mesido\examples\municipality"
                              r"\test2",
              'esdl_file_name':"GROW_withATES_Prod_install_full_GrowOptimized_costsupdated.esdl",
              'figure_folder':r"C:\git\mesido\examples\municipality\test2"
                              r"\figures"}
