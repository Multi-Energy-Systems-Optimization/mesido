...
def create_in_out_ports(asset, in_carrier, out_carrier):
...
def create_in_out_ports_influxdb_test(asset, in_carrier, out_carrier):
    ...
    in_port = esdl.InPort(id = str(uuid4()), name = "In_port", carrier = in_carrier)
    out_port = esdl.OutPort(id = str(uuid4()), name = "Out_port", carrier = out_carrier)
    in_port.profile = esdl.InfluxDBProfile(id = str(uuid4()))
    asset.port.append(in_port)
    asset.port.append(out_port)
...
def create_cost_value_profile(cost_value, unit, per_unit = None, per_multiplier = None):
    ...
    esdl_cost_profile = esdl.SingleValue(id = str(uuid4()), value = cost_value, profileQuantityAndUnit = qua_per)
...
if __name__ == "__main__":
    ...
    area_1_coord = [[52.003232185574326, 4.376142024993897], [52.002228236493224, 4.372107982635499],
                    [52.0031132980428, 4.3712711334228525], [52.00197724570447, 4.366979598999024],
                    [51.99826505971136, 4.369254112243653], [51.99945404958839, 4.373760223388673],
                    [51.995635958872874, 4.3776869773864755], [51.9963626102222, 4.380712509155274]]
    ...
    area_2_coord = [[51.99817258139733, 4.3692970275878915], [51.99932194116153, 4.373695850372315],
                    [51.99534529503086, 4.376678466796876], [51.99624370444482, 4.38081979751587],
                    [51.99490929572965, 4.381613731384278], [51.99272923553559, 4.372708797454835]]
    ...
    area_3_coord = [[51.992279997820766, 4.372987747192384], [51.994235470276436, 4.382214546203614],
                    [51.989333415199866, 4.38581943511963], [51.98688879367896, 4.37633514404297]]
    ...
    heat_commodity_primary = esdl.HeatCommodity(id = str(uuid4()), name = 'HeatCommodityPrimary', supplyTemperature = temp_supply)
    heat_commodity_ret = esdl.HeatCommodity(id = str(uuid4()), name = 'HeatCommodityRet', returnTemperature = temp_ret)
    ...
    costInf.investmentCosts = create_cost_value_profile(demand_1_inv_cost, 'EURO', per_unit = 'WATT', per_multiplier = 'MEGA')
    ...
    create_in_out_ports(heat_demand_1, in_carrier, out_carrier)
    ...
    costInf.investmentCosts = create_cost_value_profile(demand_2_inv_cost, 'EURO', per_unit = 'WATT', per_multiplier = 'MEGA')
    costInf.installationCosts = create_cost_value_profile(demand_2_inst_cost, 'EURO')

    in_carrier = esh.get_by_id_slow(id_primary_carr)
    out_carrier = esh.get_by_id_slow(id_ret_carr)
    create_in_out_ports(heat_demand_2, in_carrier, out_carrier)
    ...
    costInf.investmentCosts = create_cost_value_profile(demand_3_inv_cost, 'EURO', per_unit = 'WATT', per_multiplier = 'MEGA')
    costInf.installationCosts = create_cost_value_profile(demand_3_inst_cost, 'EURO')

    in_carrier = esh.get_by_id_slow(id_primary_carr)
    out_carrier = esh.get_by_id_slow(id_ret_carr)
    create_in_out_ports(heat_demand_3, in_carrier, out_carrier)
    ...
    costInf.investmentCosts = create_cost_value_profile(source_1_inv_cost, 'EURO', per_unit = 'WATT', per_multiplier = 'MEGA')
    costInf.installationCosts = create_cost_value_profile(source_1_inst_cost, 'EURO')
    costInf.variableOperationalCosts = create_cost_value_profile(source_1_varop_cost, 'EURO', per_unit = 'WATTHOUR', per_multiplier = 'MEGA')
    costInf.fixedOperationalCosts = create_cost_value_profile(source_1_fixop_cost, 'EURO', per_unit = 'WATT', per_multiplier = 'MEGA')

    heat_source_2.costInformation = esdl.CostInformation(id = str(uuid4()))  
    costInf = heat_source_2.costInformation
    costInf.investmentCosts = create_cost_value_profile(source_2_inv_cost, 'EURO', per_unit = 'WATT', per_multiplier = 'MEGA')
    costInf.installationCosts = create_cost_value_profile(source_2_inst_cost, 'EURO')
    costInf.variableOperationalCosts = create_cost_value_profile(source_2_varop_cost, 'EURO', per_unit = 'WATTHOUR', per_multiplier = 'MEGA')
    costInf.fixedOperationalCosts = create_cost_value_profile(source_2_fixop_cost, 'EURO', per_unit = 'WATT', per_multiplier = 'MEGA')
